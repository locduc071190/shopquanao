...
# ---------- Database setup - Định nghĩa các Model ----------
...
def initialize_database(engine):
    # Hàm này chỉ chạy 1 lần khi triển khai lần đầu
    with st.spinner("Đang kết nối và kiểm tra Database..."):
        try:
...
def load_orders():
    with SessionLocal() as session:
        # Tải orders cùng với items và product (joinedload) để tối ưu truy vấn
...
def load_stock_movements():
    with SessionLocal() as session:
        return session.query(StockMovement).options(joinedload(StockMovement.product)).order_by(StockMovement.timestamp.desc()).all()
...
# ---------- Database Helper Functions - Logic nghiệp vụ ----------
...
def add_product(name, price, cost_price, stock, notes='', image_file=None):
    ...
    if image_file:
        ...
        # Giả định thư mục images tồn tại
    ...
    with SessionLocal() as session:
        ...
        session.add(p)
        session.flush() 
        # Thêm movement ban đầu (không cần commit ở đây)
...
def update_product(product_id, name, price, cost_price, notes, image_file=None, remove_image=False):
    with SessionLocal() as session:
        ...
        # 2. Xử lý tải ảnh mới
        if image_file:
            # Xóa ảnh cũ nếu nó tồn tại và chưa bị xóa
...
def create_order(items):
    with SessionLocal() as session:
        ...
        session.add(o)
        session.flush() # Tạo order.id

        # 3. Thêm Order Items, cập nhật tồn kho, và thêm Stock Movement
        for it in items:
            p = session.get(Product, it['product_id'])
...
# --- Quản lý Sản phẩm ---
    ...
    p_to_edit = products_dict.get(st.session_state.editing_product_id)
    ...
    if p_to_edit and st.session_state.editing_product_id is not None:
        ...
        with st.form('edit_product_form', clear_on_submit=False):
            ...
            notes = st.text_area('Ghi chú', value=p_to_edit.notes or '', key='edit_notes')
            
            # Image Handling
            st.markdown('**Quản lý Ảnh**')
            current_image = p_to_edit.image_path
    ...
    if products:
        ...
        for p in products:
            ...
            cols[5].write(p.notes[:30] + '...' if len(p.notes or '') > 30 else p.notes)
        ...
        if st.button('✅ Thanh toán / Tạo đơn', type='primary'):
            try:
                if not order_items_input:
                    ...
                    status_placeholder.success(f'🎉 Đã tạo đơn **#{order_id}** thành công! Tổng cộng: **{order_total:,.0f} VND**.')
                    clear_data_cache()
